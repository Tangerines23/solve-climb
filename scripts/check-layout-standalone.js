                ...
                try {
                    ...
                    if (pageInfo.actions) {
                        try {
                            ...
                            await page.waitForTimeout(2000);
                            if (await checkOverflow(page, `${pageInfo.name} (After Manual Actions)`)) globalHasErrors = true;
                        } catch (actionErr) {
...
async function runDeepScan(page, pageName) {
    console.log(`  🕵️  Deep Scanning ${pageName}...`);
    let foundError = false;

    // 위험/제외 키워드: 페이지 이동을 유발하거나 데이터를 파괴하는 것들
    const SKIP_KEYWORDS = [
        '로그아웃', '탈퇴', '초기화', '삭제', '취소', 'Logout', 'Reset', 'Delete', 'Withdraw',
        '홈', '카테고리', '내 정보', '상점', '알림', 'Home', 'Shop', 'My Info' // 네비게이션 트리거 방지
    ];

    const currentUrl = page.url();

    // 클릭 가능한 요소들 (모달/탭/아코디언 위주)
    const buttonSelectors = ['button', '[role="button"]', '.ranking-tab-item', '.shop-tab-item'];

    for (const selector of buttonSelectors) {
        const elements = page.locator(selector);
        const count = await elements.count();

        // 너무 많으면 상위 5개만 (현실적인 시간 내 테스트)
        ...
        for (let i = 0; i < limit; i++) {
            ...
            try {
                ...
                // console.log(`    🖱️ Clicking: "${text.substring(0, 10)}..."`);
                await el.click({ timeout: 3000 });
                await page.waitForTimeout(1500);

                // 만약 페이지가 이동되었다면 다시 돌아옴
                if (page.url() !== currentUrl) {
                    await page.goto(currentUrl, { waitUntil: 'networkidle' });
                    continue;
                }

                if (await checkOverflow(page, `${pageName} (Deep click: "${text.substring(0, 10)}")`)) {
                    foundError = true;
                }
            } catch (e) {
                // 클릭 오류 무시 (팝업이 덮었거나 사라진 경우)
            }
        }
    }
    return foundError;
