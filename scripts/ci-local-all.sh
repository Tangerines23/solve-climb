...
if [ -n "${CI_SKIP_UNIT}" ]; then
  echo "[3. unit-test] Skipped (CI_SKIP_UNIT)."
else
  mkdir -p reports
...
if [ -n "${CI_SKIP_E2E}" ] || [ -n "${CI_SKIP_E2E_STABILITY}" ]; then
  ...
  if [ -z "${CI_SKIP_PLAYWRIGHT_INSTALL}" ]; then
    ...
    echo "[6. e2e-stability] Skipping Playwright install (CI_SKIP_PLAYWRIGHT_INSTALL)."
  fi
...
if [ -n "${CI_SKIP_E2E}" ]; then
  ...
  if [ -z "${CI_SKIP_E2E_STABILITY}" ]; then
    ...
    if [ -z "${CI_SKIP_PLAYWRIGHT_INSTALL}" ]; then
      ...
      echo "[7a. e2e-critical] Skipping Playwright install (CI_SKIP_PLAYWRIGHT_INSTALL)."
    fi
  fi
...
if [ -n "${CI_RUN_E2E_AUTH}" ]; then
  if [ -n "${SUPABASE_DB_PASSWORD}" ] && [ -n "${VITE_SUPABASE_URL}" ]; then
    ...
    PROJECT_REF=$(echo "$VITE_SUPABASE_URL" | sed -E 's|https://([^.]+)\.supabase\.co.*|\1|')
    npx supabase link --project-ref "$PROJECT_REF" --password "$SUPABASE_DB_PASSWORD"
    npx supabase db push --yes || (npx supabase migration repair --status reverted 20260123 && npx supabase db push --yes)
  fi
...
if [ -n "${CI_SKIP_E2E}" ] || [ -n "${CI_SKIP_E2E_VISUAL}" ]; then
  ...
  if [ -n "${SUPABASE_DB_PASSWORD}" ] && [ -n "${VITE_SUPABASE_URL}" ]; then
    ...
    PROJECT_REF=$(echo "$VITE_SUPABASE_URL" | sed -E 's|https://([^.]+)\.supabase\.co.*|\1|')
    npx supabase link --project-ref "$PROJECT_REF" --password "$SUPABASE_DB_PASSWORD" && (npx supabase db push --yes || (npx supabase migration repair --status reverted 20260123 && npx supabase db push --yes)) || true
  fi
