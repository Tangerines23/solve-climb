...
echo "[4/4] visual-regression.spec.ts --update-snapshots 실행..."
npm run test:e2e:critical -- tests/e2e/visual-regression.spec.ts --update-snapshots

SNAPSHOT="tests/e2e/snapshots/visual-regression.spec.ts-snapshots/vrt-home-page-chromium.png"
if [ -f "$SNAPSHOT" ]; then
  echo "완료: $SNAPSHOT 생성됨. 이 파일을 커밋하면 CI(e2e-critical) 시각적 회귀가 통과합니다."
else
