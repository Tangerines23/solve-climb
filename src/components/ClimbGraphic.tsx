...
export function ClimbGraphic({
  ...
  const { levelData, pathData, pathPoints, svgHeight } = useMemo(() => {
    ...
    for (let i = 0; i < totalLevels; i++) {
      ...
      const baseY = startY - (startY - endY) * progress;
      
      // S자 패턴: sin 함수로 좌우로 움직임 (더 부드러운 곡선)
      const offsetX = Math.sin(progress * Math.PI * 2.2) * (svgWidth * 0.18);
      const offsetY = Math.cos(progress * Math.PI * 1.3) * (calculatedSvgHeight * 0.03);
    ...
    if (points.length > 0) {
      ...
      for (let i = 0; i < points.length - 1; i++) {
        ...
        const dx1 = (next.x - prev.x) * 0.15;
        const dy1 = (next.y - prev.y) * 0.15;
        const dx2 = (after.x - curr.x) * 0.15;
        const dy2 = (after.y - curr.y) * 0.15;
    ...
    >
      ...
      <div className="level-map-mountains">
        ...
        >
          ...
          {/* 앞쪽 산 (가장 선명하게) */}
          <polygon
            points="0,300 100,180 200,300"
            fill={categoryColor}
            opacity="0.8"
            className="mountain-bg-near"
          />
          <polygon
            points="200,300 300,160 400,300"
            fill={categoryColor}
            opacity="0.8"
            className="mountain-bg-near"
          />
