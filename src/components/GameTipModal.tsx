    ...
    if (category === '기초' || category === '대수' || category === '논리' || category === '심화') {
      ...
      if (category === '기초') {
        if (subTopic === 'World2') {
          ...
          title = '공학 및 응용 팁';
          icon = '💻';
          description = '2진수(비트)에서 출발하여 논리 게이트, 메모리 단위, 자료구조, 보수 연산과 CPU 계산의 흐름을 이해하세요.';
          tips = {
            1: {
              title: '1레벨: 2진수 -> 10진수',
              tip: '이진법 변환',
              example: '2진수 1101 -> 10진수 13',
              strategy: '컴퓨터가 모든 데이터를 저장하는 기본 형태(0과 1)인 2진수입니다. 각 자리 가중치(8, 4, 2, 1)를 1이 켜진 자리에 더하세요.',
            },
            2: {
              title: '2레벨: 10진수 -> 2진수',
              tip: '이진법 역변환',
              example: '10진수 10 -> 2진수 1010',
              strategy: '우리가 쓰는 10진수를 컴퓨터의 언어(2진수)로 변환하는 과정입니다. 10진수에서 가중치 값을 큰 것부터 차감해 나가며 비트를 채웁니다.',
            },
            3: {
              title: '3레벨: 16진수 기초',
              tip: '헥사 코드 이해',
              example: '16진수 1A -> 10진수 26',
              strategy: '2진수 비트열이 너무 길어지므로, 사람이 읽기 편하게 4비트씩 묶어 압축한 것이 16진수입니다. A~F는 10~15를 뜻합니다.',
            },
            4: {
              title: '4레벨: 논리 게이트 AND',
              tip: '논리곱 AND',
              example: '1 AND 0 -> 0',
              strategy: '비트를 제어하는 기본 논리 게이트입니다. 두 입력 신호가 모두 켜져(1) 있어야만 통과(1)시키는 스위치 역할을 합니다.',
            },
            5: {
              title: '5레벨: 논리 게이트 OR',
              tip: '논리합 OR',
              example: '1 OR 0 -> 1',
              strategy: '입력 신호 중 하나라도 켜져(1) 있으면 통과(1)시키는 논리 게이트입니다.',
            },
            6: {
              title: '6레벨: 논리 게이트 NOT',
              tip: '부정 NOT',
              example: 'NOT 1 -> 0',
              strategy: '입력 신호를 거꾸로 뒤집는(0->1, 1->0) 가장 단순한 반전 게이트입니다.',
            },
            7: {
              title: '7레벨: 논리 게이트 XOR',
              tip: '배타적 논리합 XOR',
              example: '1 XOR 1 -> 0',
              strategy: '두 신호가 서로 다를 때만 1을 출력합니다. 컴퓨터 계산기(가산기) 회로에서 덧셈의 합(Sum)을 구하는 핵심 게이트입니다.',
            },
            8: {
              title: '8레벨: 메모리 단위 기본',
              tip: 'Byte와 KB 환산',
              example: '4096 바이트 -> 4 KB',
              strategy: '이 비트와 논리 회로들이 대규모로 엮여 구성된 저장 장치, 즉 메모리의 용량을 측정하는 단위(1 KB = 1024 Byte)의 시작입니다.',
            },
            9: {
              title: '9레벨: 메모리 단위 심화',
              tip: 'KB와 MB 환산',
              example: '2048 KB -> 2 MB',
              strategy: '킬로바이트(KB)를 넘어 메가바이트(MB) 등 더 거대한 대용량 데이터를 다루기 위해 단위를 확장한 개념입니다 (1 MB = 1024 KB).',
            },
            10: {
              title: '10레벨: 자료구조 스택',
              tip: 'LIFO (Last-In, First-Out)',
              example: 'push(5), push(3), pop() -> 남은 것 5',
              strategy: '메모리에 쌓이는 데이터를 관리하는 기초적인 선형 자료구조입니다. 나중에 들어온 것이 먼저 나가는(LIFO) 상자 쌓기식 구조입니다.',
            },
            11: {
              title: '11레벨: 자료구조 큐',
              tip: 'FIFO (First-In, First-Out)',
              example: 'enqueue(5), enqueue(3), dequeue() -> 남은 것 3',
              strategy: '네트워크 패킷이나 대기열을 순차 처리하는 자료구조입니다. 먼저 들어온 데이터가 먼저 처리되어 나갑니다(FIFO).',
            },
            12: {
              title: '12레벨: 보수법 - 1의 보수',
              tip: '비트 반전 연산',
              example: '0101의 1의 보수 -> 1010',
              strategy: 'CPU가 뺄셈 회로를 따로 만들지 않고 덧셈 회로만으로 뺄셈을 하기 위해, 일단 모든 비트를 반전시키는 준비 단계입니다.',
            },
            13: {
              title: '13레벨: 보수법 - 2의 보수',
              tip: '실제 음수 비트 표현',
              example: '0110의 2의 보수 -> 1010',
              strategy: '1의 보수(비트 반전) 결과에 마지막으로 1을 더해줍니다. 이로써 컴퓨터는 음수(-) 값을 완벽하게 표현하고, 덧셈기 하나만으로 뺄셈을 처리할 수 있게 됩니다.',
            },
            14: {
              title: '14레벨: 2진수의 덧셈',
              tip: '이진수 캐리 연산',
              example: '011 + 010 -> 101',
              strategy: '앞에서 배운 논리 게이트와 보수를 활용해, 실제로 CPU 내의 가산기(ALU)가 이진수 덧셈과 뺄셈을 처리하는 연산 과정입니다.',
            },
            15: {
              title: '15레벨: 이진 소수의 연산',
              tip: '이진 소수의 덧셈과 변환',
              example: '0.1 + 1.1 -> 10.0 (10진수로 0.5 + 1.5 = 2.0)',
              strategy: '정수 연산을 넘어, 컴퓨터가 소수점 이하의 실수(부동소수점)를 2진수로 나타내고 계산하는 원리입니다. 0.1은 1/2(0.5)을 뜻합니다.',
            },
          };
        } else {
          title = '사칙연산 팁';
