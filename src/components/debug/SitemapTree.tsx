        ...
        children: [
            {
                ...
                children: mathCategories.map(cat => ({
                    ...
                    children: mathWorlds.map(world => {
                        ...
                        const hasLevels = (APP_CONFIG.LEVELS[world.id as keyof typeof APP_CONFIG.LEVELS] as any)?.[cat.id]?.length > 0;
            ...
            },
            {
                ...
                children: [
                    { name: '🗾 언어의 산 (Language)', path: '/category-select?mountain=language' },
                ]
            }
        ]
    };
...
const TreeNode = ({ node, depth = 0, ancestorPaths = [] }: { node: SiteNode; depth?: number; ancestorPaths?: string[] }) => {
    ...
    const handleMacroClick = (e: React.MouseEvent) => {
        ...
        console.log(`%c[Macro] 🏁 Full Trace Started: ${node.name}`, 'color: #3b82f6; font-weight: bold; font-size: 14px;');
        ...
        fullTour.forEach((path, index) => {
            setTimeout(() => {
                ...
                console.log(`%c[Macro] ${emoji} Step ${index + 1}/${fullTour.length}: ${path}`, 'color: #10b981');
    ...
    return (
        <div style={{ marginLeft: depth * 20, marginTop: 'var(--spacing-xs)' }}>
            ...
            >
                ...
                {/* 노드 이름 */}
                <span style={{ fontWeight: 500, color: 'white', flex: 1 }}>
                    {node.name}
                </span>
            ...
            {/* 자식 노드 렌더링 */}
            {hasChildren && isOpen && (
                <div style={{ borderLeft: '1px dashed var(--color-bg-tertiary)', marginLeft: 'var(--spacing-small-alt)' }}>
                    {node.children!.map((child, index) => (
                        <TreeNode
                            key={index}
                            node={child}
                            depth={depth + 1}
                            ancestorPaths={currentLineage}
                        />
                    ))}
                </div>
            )}
...
export function SitemapTree() {
    return (
        <div className="sitemap-tree" style={{
