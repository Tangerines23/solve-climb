...
// Level 6: 삼각형 넓이 (3B1B 평행사변형 합성 애니메이션 - 제시 1, 2, 3 비교 모드)
// 방향키 (←, →) 또는 UI 클릭으로 제시 1 / 제시 2 / 제시 3 비교 전환 가능
export const ManimLevel6Visualizer: React.FC = React.memo(() => {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    if (e.key === 'ArrowLeft') {
      setProposal((prev) => ((prev - 1 + 3) % 3) as ProposalType);
    } else if (e.key === 'ArrowRight') {
      setProposal((prev) => ((prev + 1) % 3) as ProposalType);
    }
  }, []);
  ...
  // 평행사변형 완성 목표 점4 좌표: (5, 50)
  const targetP4: Point = { x: 5, y: 50 };

  // -------------------------------------------------------------
  // 복제 삼각형 (1-2-4) 꼭짓점 좌표 계산 (제시 1, 2, 3 별 로직)
  // -------------------------------------------------------------
  ...
  if (stepIndex === 0) {
    ...
    if (proposal === 0) {
      // ---------------------------------------------------------
      // [제시 1: 이동하는 피봇 회전]
      // 점2에서 회전을 시작하며, 회전하는 동안 피봇점이 점1으로 이동
      // ---------------------------------------------------------
      ...
      // 점4를 점3에 겹쳐 생성 후, 반대쪽 목표 점4(5, 50)로 직선 이동
      // ---------------------------------------------------------
      ghostP1 = { ...p1 };
      ghostP2 = { ...p2 };
      ghostP4 = {
      ...
      // ---------------------------------------------------------
      // [제시 3: 빗변 축(변1-2) 반사 뒤집기 (Hinge Flip)]
      // 빗변 변1-2를 경첩 축으로 삼아 180도 대칭 반사 폅침
      // ---------------------------------------------------------
      ghostP1 = { ...p1 };
      ghostP2 = { ...p2 };

      // 빗변 축 기준 대칭 반사 계산 + 회전 궤적 감성
  ...
  let badgeName = `${proposalTitles[proposal]} - 1. 삼각형 (b=12, h=8)`;
  let caption = (
  ...
  if (stepIndex === 1) {
    badgeName = `${proposalTitles[proposal]} - 2. 평행사변형 완성!`;
    caption = (
      <div className="geo-stat-highlights">
    ...
    badgeName = `${proposalTitles[proposal]} - 3. 절반(÷ 2) 넓이`;
    caption = (
      <div className="geo-stat-highlights">
        <span className="geo-stat-item" style={{ color: '#38bdf8' }}>
          {baseVal} × {heightVal}
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', marginBottom: '4px' }}>
        {([0, 1, 2] as ProposalType[]).map((idx) => (
          <button
            key={idx}
            onClick={() => setProposal(idx)}
            style={{
              padding: '2px 8px',
              fontSize: '10px',
              fontWeight: proposal === idx ? 800 : 500,
              borderRadius: '4px',
              border: proposal === idx ? '1px solid #38bdf8' : '1px solid rgba(255,255,255,0.2)',
              backgroundColor: proposal === idx ? 'rgba(56, 189, 248, 0.2)' : 'transparent',
              color: proposal === idx ? '#38bdf8' : 'rgba(255,255,255,0.6)',
              cursor: 'pointer',
            }}
          >
            제시 {idx + 1}
