...
describe('useDailyRewardStore', () => {
  ...
  it('should not show modal if no session exists', async () => {
    vi.mocked(supabase.auth.getSession).mockResolvedValue(createGetSessionMock(null));
  ...
  it('should show modal and set reward results on successful daily login RPC', async () => {
    ...
    vi.mocked(supabase.auth.getSession).mockResolvedValue(createGetSessionMock(mockSession));
