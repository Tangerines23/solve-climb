...
export function useCategoryList() {
  ...
  const handleToggleFavorite = useCallback((e: React.MouseEvent, mountainId: string, mountainName: string) => {
    e.stopPropagation();
    addFavorite({
      type: 'category',
  ...
  const handleMountainClick = useCallback((mountainId: string) => {
  ...
  const getMountainAltitudeInfo = useCallback((mountainId: string) => {
