...
export function useLevelList(world: string, category: string, tier: Tier = 'normal') {
  ...
  const nextLevel = useMemo(() => getNextLevel(world, category, tier), [getNextLevel, world, category, tier]);
  const progress = useMemo(() => getLevelProgress(world, category, tier), [getLevelProgress, world, category, tier]);

  const isUnderDevelopment = useCallback((level: number) => {
  ...
  const getBestScore = useCallback((level: number): number | null => {
