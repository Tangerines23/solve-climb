  ...
  Object.values(progress).forEach((categoryData: any) => {
    Object.values(categoryData).forEach((subTopicData: any) => {
      Object.values(subTopicData).forEach((levelRecord: any) => {
        ...
        if (scoreAlt.ok) {
          total = total.add(scoreAlt.value);
        }
      });
    });
  });

  const totalAltitude = total.meters;
...
export function calculateSubTopicAltitude(category: string, subTopic: string): number {
  ...
  const total = levelProgress.reduce(
    (sum: Altitude, record: LevelRecord) => {
      const score = getBestScore(record);
      const scoreAlt = Altitude.create(score);
      return scoreAlt.ok ? sum.add(scoreAlt.value) : sum;
    },
    (Altitude.create(0) as { ok: true; value: Altitude }).value
  );
