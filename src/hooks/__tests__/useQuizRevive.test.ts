...
describe('useQuizRevive', () => {
    const mockProps = {
        ...
        inventory: [{ id: 1, code: 'flare', count: 1 }], // flare for survival
        minerals: 1000,
        consumeItem: vi.fn(),
        ...
        animations: { setIsError: vi.fn() } as any,
    ...
    it('handleRevive (Time Attack) should add time', async () => {
        const timeAttackProps = {
            ...
            inventory: [{ id: 2, code: 'last_spurt', count: 1 }],
        };
        const setTimeLimitMock = vi.fn();
        (useQuizStore.getState as any).mockReturnValue({ setTimeLimit: setTimeLimitMock });
