    ...
    const checkAndAwardBadges = useCallback(async (userId: string, stats: HistoryStats) => {
        ...
        for (const badge of BADGE_DEFINITIONS) {
            ...
            if (badge.minAccuracy) {
                ...
                if (totalGamesPlayed >= (badge.minGames || 10) && stats.averageAccuracy >= badge.minAccuracy) {
                    qualified = true;
                }
                if (totalGamesPlayed >= (badge.minGames || 10) && stats.averageAccuracy >= badge.minAccuracy) {
                    qualified = true;
            ...
            if (badge.goalThemePart && badge.goalLevel) {
                const hasClearedLevel = stats.categoryLevels.some(cat =>
                    cat.themeId.includes(badge.goalThemePart) && cat.level >= badge.goalLevel
                );
                if (hasClearedLevel) {
                    qualified = true;
                }
