...
export function useBoundaryDebugBridge() {
  ...
  const handleStaminaSet = useCallback(async (value: number) => {
  ...
  const handleMineralsSet = useCallback(async (value: number) => {
    if (isUpdating) return;
  ...
  const handleTierSet = useCallback(async (level: number) => {
    if (isUpdating) return;
  ...
  const handleMasteryScoreSet = useCallback(async (score: number) => {
    if (isUpdating) return;
