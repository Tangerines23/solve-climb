...
export function AuthCallbackPage() {
    ...
    const handleAuthCallback = async () => {
      try {
        ...
        if (error) {
          ...
          if (isTossEnvironment()) {
            console.warn(
              '[Toss] 인증 콜백 처리 실패 - 토스 앱 환경에서 Supabase 인증이 제한될 수 있습니다.'
            );
          }
  ...
  return (
    ...
    >
      <div style={{ fontSize: '48px', marginBottom: 'var(--spacing-lg)' }}>{getStatusIcon()}</div>
      <div style={{ fontSize: '18px', marginBottom: 'var(--spacing-md)' }}>{message}</div>
      {status === 'loading' && (
        <div
          style={{
            fontSize: '14px',
