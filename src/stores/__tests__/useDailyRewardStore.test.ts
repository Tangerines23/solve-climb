...
describe('useDailyRewardStore', () => {
  ...
  it('should handle Supabase RPC errors gracefully without crashing', async () => {
    const mockSession = { user: { id: 'test-user' } };
    vi.mocked(supabase.auth.getSession).mockResolvedValue({ data: { session: mockSession }, error: null } as any);
    vi.mocked(supabase.rpc).mockResolvedValue({ data: null, error: { message: 'Database Error' } } as any);
    ...
    expect(result.current.isLoading).toBe(false);
    expect(result.current.showModal).toBe(false);
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('RPC Error'), expect.anything());
    
    consoleSpy.mockRestore();
  });

  it('should close modal and clear the reward result state', () => {
    useDailyRewardStore.setState({ showModal: true, rewardResult: { success: true, message: 'Test' } });
