    ...
    switch (level) {
        ...
        case 9:
            return {
                ...
                hintType: 'calculus',
                hintData: { type: 'derivative' }
            };
        case 10:
            return {
                ...generateDerivativeProblem('coefficient'),
                hintType: 'calculus',
                hintData: { type: 'derivative' }
            };
        case 11:
...
function generateFunctionProblem(): CalculusProblem {
    ...
    if (type === 1) {
        ...
        return { question: `f(x) = x + ${a}, f(${x}) = ?`, answer: x + a };
    } else {
        return { question: `f(x) = x², f(${x}) = ?`, answer: x * x };
    }
}

function generateDerivativeProblem(type: 'basic' | 'coefficient' | 'linear' | 'const'): CalculusProblem {
    ...
    switch (type) {
        ...
        case 'linear':
            return { question: `d/dx(${a}x) 의 값은?`, answer: a };
        case 'const':
            return { question: `d/dx(${getRandomInt(1, 100)}) 의 값은?`, answer: 0 };
    }
}
...
function generateIntegralProblem(type: 'power' | 'simple'): CalculusProblem {
    ...
    if (type === 'power') {
        ...
        return { question: `∫ ${a} dx , x=1 일 때 값은? (C=0)`, answer: a };
    }
}
