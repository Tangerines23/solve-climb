-- ============================================================================
-- 매핑 테이블 수정: 실제 game_records 데이터에 맞춰 업데이트
-- 작성일: 2025.12.25
-- ============================================================================
-- 문제: 
...
-- 마이그레이션 함수에서 하이픈을 제거하여 기존 timeattack과 매칭하도록 처리

-- 3. migrate_existing_records 함수 수정
-- 하이픈 문제 해결: time-attack을 timeattack으로 변환하거나 둘 다 지원
...
BEGIN
  ...
  -- ⚠️ mode 변환: time-attack -> timeattack (하이픈 제거)
  ...
  INNER JOIN public.mode_mapping mm 
    ...
    OR mm.mode_id = gr.mode  -- 하이픈 없는 경우도 지원
  WHERE gr.score > 0
