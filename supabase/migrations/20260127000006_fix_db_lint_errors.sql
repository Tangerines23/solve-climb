-- ============================================================================
-- DB Lint ?�러 ?�정 �??�수 고도??(2026.01.27)
-- 1. check_and_award_badges: theme_mapping ?�존???�거 �?문자??기반 매칭?�로 ?�정
-- 2. recover_stamina_ads: 미사??변???�거
-- 3. debug_run_play_scenario: 변??가리기(Shadowing) 문제 ?�결
-- ============================================================================

-- 1. check_and_award_badges ?�정
...
BEGIN
    FOR v_badge_def IN SELECT * FROM public.badge_definitions LOOP
        ...
        -- 1. 카테고리 ?�체 ?�리??(?�수 ?�벨 목록???�는 경우)
        IF v_badge_def.required_levels IS NULL THEN
            -- ?? 'math' 카테고리 ?�의 모든 ?�벨??최소 1개라???�수가 ?�는지 ?�인
        ...
        -- 2. ?�정 ?�마/?�벨 ?�리??(?�수 ?�벨 목록???�는 경우)
        ELSE
            -- theme_mapping 조인 ?�거?�고 category_id/subject_id 기반?�로 직접 ?�인
...
-- 2. recover_stamina_ads ?�정 (미사??v_current_stamina ?�거)
...
-- 3. debug_run_play_scenario ?�정 (v_level 중복 ?�언 ?�거)
...
-- 4. debug_generate_dummy_record ?�정 (theme_mapping ?�거 �?string ID ?�??
