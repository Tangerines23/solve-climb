-- Fix answer JSON key in submit_game_result RPC function (support 'answer', 'correct_answer', 'correctAnswer')
...
BEGIN
  ...
  FOR i IN 0..(v_total_questions - 1) LOOP
    ...
    IF v_question IS NOT NULL THEN
      -- Fix: 지원 가능한 모든 정답 키 (answer, correct_answer, correctAnswer) 호환 검출
