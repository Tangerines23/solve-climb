-- Migration: Add ON CONFLICT to profiles insert & add overload wrappers to prevent 409 Conflict
-- Date: 2026-07-27

-- Drop existing functions first to prevent 42P13 parameter defaults error
...
-- 1. Fix handle_daily_login with ON CONFLICT (id) DO UPDATE
...
-- 2. Fix secure_reward_ad_view with ON CONFLICT (id) DO UPDATE
...
-- 3. Fix check_and_recover_stamina with ON CONFLICT (id) DO UPDATE
...
-- 4. Add 0-arg overload for check_and_recover_stamina()
