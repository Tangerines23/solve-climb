...
test.describe('CORE BUSINESS SCENARIO - 게임 플레이부터 랭킹 반영까지', () => {
  test('수학 퀴즈를 풀고 결과를 제출하여 랭킹을 확인할 수 있어야 한다', async ({ page }) => {
    ...
    // 로딩 오버레이 숨김 대기 (클릭 차단 방지)
    await page
      .waitForSelector('#loading-check', { state: 'hidden', timeout: 10000 })
      .catch(() => {});

    // 일일 보상 모달이 있으면 닫기 (클릭 차단 방지)
    await page
      .locator('.daily-reward-confirm-button')
      .click({ timeout: 5000 })
      .catch(() => {});
    ...
    await level1Button.evaluate((el) => (el as HTMLElement).click());
    ...
    // 스태미나 부족 모달이 있으면 "지친 상태로 진행" 선택
    console.log('[E2E] Checking for stamina modal...');
    const tiredProceedBtn = page.locator('button:text-is("지친 상태로 진행")');
    try {
      await tiredProceedBtn.waitFor({ state: 'visible', timeout: 5000 });
      console.log('[E2E] Stamina low. Proceeding while tired...');
      await tiredProceedBtn.click();
    } catch {
      console.log('[E2E] No stamina modal appearing.');
    }

    // 6. 퀴즈 풀기 (최소 1문제) — RPC/스태미나 반영 후 UI 로드까지 여유
    console.log('[E2E] Step 6: Solving quiz...');
    await page.waitForSelector('.problem-text', { timeout: 25000 });
    ...
    const hasMyItem = (await myRankItem.count()) > 0;
    if (hasMyItem) {
      ...
      console.log(
        '[E2E] My rank item not in list (submit_game_result may have failed); ranking page loaded.'
      );
    }
