...
test.describe('NETWORK STRESS TEST - Latency Hell', () => {
  test('고점연(5초) 환경에서도 핵심 시나리오가 안정적으로 작동해야 한다', async ({ page }) => {
    test.setTimeout(180_000); // 3 minutes total timeout for stress test
    ...
    await page.goto('/', { timeout: 120_000 }); // 지연이 있으므로 타임아웃 넉넉히

    // 2. 등반하기 버튼 클릭 (로딩 UI 확인 목적)
    const climbButton = page.locator('.category-climb-button').first();
    await expect(climbButton).toBeVisible({ timeout: 60_000 });
    ...
    await page.waitForURL(/.*category-select.*/, { timeout: 90_000 });
